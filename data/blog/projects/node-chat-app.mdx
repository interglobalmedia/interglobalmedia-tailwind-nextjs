...
-   **_Each socket_** in `Socket.io` is **_identified_** by a `random`,
    ...
    **_better use_** of **_thi_**s**_ `Socket.io` _**feature**_ than
    _**creating**_ a `room` that `multiple users` can _**join**_. _**As**_ a
    _**result**_, one can _**create**_ `either` a `public chat` in which the
    `messages` are _**shared**_ with `everyone` _**everywhere**_, or
    _**create**_ a `private chat` _**with**_ `one user` _**at**_ a
    _**time\*\*\*.
